...
public class TextFrame extends ResizableFrame {
	...
	public TextFrame() {
		...
		readFile("twl/textarea/demo.html");
	...
	void readFile(String name) {
		try {
			textAreaModel.readHTMLFromURL(ResourceLoader.getResource(name));

			StyleSheet styleSheet = new StyleSheet();
			for(String styleSheetLink : textAreaModel.getStyleSheetLinks()) {
				try {
					styleSheet.parse(ResourceLoader.getResource("twl/textarea/" + styleSheetLink));
				} catch(IOException ex) {
					Logger.getLogger(TextFrame.class.getName()).log(Level.SEVERE,
			...
			Logger.getLogger(TextFrame.class.getName()).log(Level.SEVERE, "Can't read HTML: " + name, ex);
		}
	}
